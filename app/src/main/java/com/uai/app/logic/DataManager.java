...
public class DataManager {
    ...
    public Map<String, Set<Libro>> getBooksByColum(Tittles columName){
        ...
        for (Libro p : this.data){
            ...
            try {
                ...
                Set<Libro> librosB = resultados.get(filterName);
                //Significa que debo crear si es true
                if (AppUtils.isNull(librosB)){
                    //uso un set para evitar repetidos
                    librosB = new HashSet<Libro>();
                }
                librosB.add(p);
                resultados.put(filterName, librosB);
